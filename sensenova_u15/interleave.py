...
class SenseNovaInterleaveSession:
    ...
    def _append_image(self, prefix, image):
        hidden, prefix.keys, prefix.values, prefix.time = self.model.append_interleave_image(
            image,
            prefix.keys,
            prefix.values,
            prefix.time,
...
def prefix_arguments(metadata, device, dtype, image_only):
    ...
    references = metadata.get("sensenova_reference_images")
    ...
    if references:
        ...
        reference_grids = [
            (
                ...
                max(1, (image.shape[-1] + MERGED_PATCH_SIZE - 1) // MERGED_PATCH_SIZE),
            )
        ...
        references = [image.to(device=device, dtype=dtype) for image in references]
...
def _parse_interleave_parts(text, num_images):
    ...
    while cursor < len(text):
        matches = [(index, tag) for tag in tags if (index := text.find(tag, cursor)) >= 0]
        if not matches:
            value = text[cursor:].strip()
            if value:
                parts.append({"type": "think" if in_think else "text", "text": value})
            break
