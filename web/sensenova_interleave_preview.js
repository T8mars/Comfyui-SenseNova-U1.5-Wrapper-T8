...
function renderThinking(container, message) {
    const text = previewText(message);
    container.textContent = text || "(no thinking output)";
    container.classList.toggle("sn-interleave-placeholder", !text);
}
...
function registerThinkingPreview(nodeType) {
    ...
    nodeType.prototype.onExecuted = function (message) {
        ...
        renderThinking(this._snThinkingContainer, message);
        this.setDirtyCanvas?.(true, true);
    };
}

function registerInterleavePreview(nodeType) {
    const onNodeCreated = nodeType.prototype.onNodeCreated;
    nodeType.prototype.onNodeCreated = function () {
        const result = onNodeCreated?.apply(this, arguments);
        const container = document.createElement("div");
...
app.registerExtension({
    ...
    async beforeRegisterNodeDef(nodeType, nodeData) {
        if (
            nodeData?.name !== "SenseNovaThinkingPreview" &&
            nodeData?.name !== "SenseNovaInterleavePreview"
        ) {
            return;
        }
        ensureStyles();
        if (nodeData.name === "SenseNovaThinkingPreview") {
            ...
            registerInterleavePreview(nodeType);
        }
    },
});
